...
import { fetchPrayerRooms } from "@/lib/api";
...
export default function Map({ filters, userLocation, searchLocation, onMarkerClick, language = 'en' }: MapProps) {
  ...
  useEffect(() => {
    ...
    const allAmenities = [...prayerRooms];

    // Filter amenities based on current filters
    const filteredAmenities = allAmenities.filter(amenity => {
      if (!filters.showPrayerRooms) return false;
      if (!filters.showMultiFaith && !amenity.denomination) return false;
      if (!filters.showIslamic && amenity.denomination !== 'muslim') return false;
    ...
    filteredAmenities.forEach(amenity => {
      ...
      let iconColor = '#9333ea'; // Purple for prayer rooms
      let emoji = '🕌';

      if (amenity.denomination === 'muslim') {
        iconColor = '#059669'; // Green for Islamic
        emoji = '☪️';
      } else if (amenity.denomination === 'christian') {
        iconColor = '#2563eb'; // Blue for Christian
        emoji = '✝️';
      ...
      const marker = L.marker([lat, lng], { icon: customIcon })
        .bindTooltip(amenity.name || 'Prayer Room', {
          permanent: false,
          direction: 'top',
          offset: [0, -15]
        })
    ...
    console.log(`🕌 Added ${filteredAmenities.length} prayer room markers`);
  }, [prayerRooms, filters, onMarkerClick]);
